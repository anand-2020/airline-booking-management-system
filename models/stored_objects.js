...
BEGIN
    ...
    SET FROM_TS = TIMESTAMP(FROM_DATE, ADDTIME(CURRENT_TIME(), '02:00:00'));
    SET TO_TS = TIMESTAMP(DATE_ADD(FROM_DATE, INTERVAL 7 DAY), ADDTIME(CURRENT_TIME(), '02:00:00'));
    ...
    SELECT P.FLIGHT_ID,
        TIMESTAMP(D.DEPARTURE_DATE, ADDTIME(P.DEPARTURE_TIME, ADDTIME(D.DELAYED_BY, SOURCE_OFFSET))) AS DEP_TS,
        P.DURATION,
        TIMESTAMP(D.DEPARTURE_DATE, ADDTIME(ADDTIME(P.DEPARTURE_TIME, D.DELAYED_BY), ADDTIME(P.DURATION, DEST_OFFSET))) AS ARR_TS,
...
DROP TRIGGER IF EXISTS ADD_FLIGHT_PATH $$
CREATE TRIGGER ADD_FLIGHT_PATH
BEFORE INSERT ON FLIGHT_PATH
FOR EACH ROW
BEGIN
    IF NEW.DEPARTURE_TIME < '00:00:00' OR NEW.DEPARTURE_TIME >= '24:00:00' THEN
        SIGNAL SQLSTATE '45001' SET MESSAGE_TEXT = 'INCORRECT DEPARTURE_TIME';
    ELSEIF NEW.SOURCE_ID = NEW.DESTINATION_ID THEN
        SIGNAL SQLSTATE '45002' SET MESSAGE_TEXT = 'SOURCE_ID AND DESTINATION_ID CAN NOT BE THE SAME';
    ELSEIF NEW.LEASE_EXPIRY <= CURDATE() THEN
